...
# PATCH (per your last message): Fix ONLY
#   (1) Schedule table (severely wrong matchups)
#   (2) Team tab box score selection showing "No game log available..."
...
# =========================
# PATCH 1: SCOREBOARD / SCHEDULE FIX
# =========================
@st.cache_data(ttl=TEAM_CTX_TTL_SECONDS, show_spinner=False)
def get_daily_scoreboard(game_date: dt.date) -> pd.DataFrame:
    """
    Returns a clean, reliable 1-row-per-game table:
      GAME_ID, DATE, MATCHUP (AWAY @ HOME), STATUS_TEXT, STATUS_DISPLAY, HOME_PTS, AWAY_PTS
    Fix: build matchup using GameHeader HOME_TEAM_ID/VISITOR_TEAM_ID + static team abbrev,
         and pull PTS from LineScore keyed by TEAM_ID. No fragile merges/renames.
    """
    ...
    for r in game_header.itertuples(index=False):
        ...
        # If either team_id missing, skip
        ...
        # Display: Final score if status indicates final AND points exist, else show status text
...
def get_schedule_range(start_date: dt.date, days: int) -> pd.DataFrame:
    """
    Fix: returns exactly the schedule table you asked for: one row per matchup with
    MATCHUP + start/status (and final score when completed).
    """
    all_days = []
    for i in range(days):
...
# =========================
# PATCH 2: TEAM GAMELOG FALLBACK FIX
# =========================
@st.cache_data(ttl=TEAM_CTX_TTL_SECONDS, show_spinner=False)
def get_team_gamelog(team_id: int, season: str) -> pd.DataFrame:
    """
    Primary: nba_api TeamGameLog (ideal because it includes MATCHUP, WL, GAME_ID).
    Fallback (fix for Streamlit Cloud empties): build gamelog from scoreboard across last 200 days,
    filtering games that include TEAM_ID, and deriving WL from scores.
    """
    # Primary
    try:
        frames = _retry_api(
    ...
    for i in range(200):
        ...
        # need to know if this team is in the matchup
        for r in day.itertuples(index=False):
            ...
            # Determine WL using scores when available
            wl = ""
...
def league_tab():
    ...
    # Schedule section (PATCH: now correct matchups)
...
def team_tab():
    ...
    # PATCH: with fallback, this should no longer be empty on Streamlit Cloud
    ...
    # --- everything below remains untouched from your previous version ---
    st.divider()
