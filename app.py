...
def ensure_columns(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    """
    Ensure all columns in `cols` exist in df. Missing ones are added as NaN.
    Returns a copy.
    """
    out = df.copy()
    for c in cols:
...
def get_standings_full(season: str) -> pd.DataFrame:
    ...
    if winpct_col:
        out["WIN_PCT"] = pd.to_numeric(st_df[winct_pct_col := winpct_col], errors="coerce")
    else:
...
def get_league_team_summary(season: str) -> pd.DataFrame:
    ...
    base_cols = ["TEAM_ID", "TEAM_NAME", "TEAM_ABBREVIATION", "PTS", "REB", "AST"]
    base = ensure_columns(base, base_cols)
    base_min = base[base_cols].copy()

    adv_cols = ["TEAM_ID", "OFF_RATING", "DEF_RATING", "NET_RATING", "PACE"]
    adv = ensure_columns(adv, adv_cols)
    adv_min = adv[adv_cols].copy()

    # Normalize dtypes for merge keys across all three DataFrames
    for df in (standings, base_min, adv_min):
    ...
    # Drop any rows with missing TEAM_ID to avoid type/NaN merge issues
    standings = standings.dropna(subset=["TEAM_ID"]).copy()
    base_min = base_min.dropna(subset=["TEAM_ID"]).copy()
    adv_min = adv_min.dropna(subset=["TEAM_ID"]).copy()

    # Enforce common dtype on TEAM_ID to prevent ValueError in merge
    standings["TEAM_ID"] = standings["TEAM_ID"].astype("Int64")
    base_min["TEAM_ID"] = base_min["TEAM_ID"].astype("Int64")
    adv_min["TEAM_ID"] = adv_min["TEAM_ID"].astype("Int64")
...
def team_dashboard():
    ...
    def _select_roster_columns(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.copy()
        df = ensure_columns(df, list(colmap.keys()))
    ...
    trad = ensure_columns(trad, TRAD_WANTED)
    adv = ensure_columns(adv, ADV_WANTED)

    trad_g = trad[TRAD_WANTED].copy()
    adv_g = adv[ADV_WANTED].copy()
...
def league_dashboard():
    ...
    with teams_tab:
        ...
        if team_summary.empty:
            ...
            df = ensure_columns(df, display_cols)
