...
def _retry_api(endpoint_cls, kwargs, timeout=REQUEST_TIMEOUT, retries=MAX_RETRIES, sleep=0.8):
    ...
    # If we got here, bubble up last error
    raise last_err
...
# ----------------------- League helpers: scoreboard & standings -----------------------
...
def get_daily_scoreboard(date_obj: datetime.date):
    ...
    # Force abbrev columns to clean string dtype (fixes UFuncNoLoopError)
...
def get_standings(season: str):
    """Return East/West standings for the given season."""
    try:
        frames = _retry_api(
    ...
    # Try to identify columns defensively
    cols = st_df.columns
    ...
    required = [team_name_col, team_abbr_col, conf_col, wins_col, losses_col]
    if any(c is None for c in required):
        return pd.DataFrame(), pd.DataFrame()

    out = pd.DataFrame(
        {
            "TEAM": st_df[team_name_col].astype(str),
            "ABBR": st_df[team_abbr_col].astype(str),
    ...
    east = out[out["CONF"].str.upper().str.contains("EAST")].copy()
    west = out[out["CONF"].str.upper().str.contains("WEST")].copy()

    east = east.sort_values(["WIN_PCT", "W"], ascending=[False, False]).reset_index(drop=True)
    west = west.sort_values(["WIN_PCT", "W"], ascending=[False, False]).reset_index(drop=True)

    east.insert(0, "RANK", np.arange(1, len(east) + 1))
    west.insert(0, "RANK", np.arange(1, len(west) + 1))
...
def get_league_team_advanced(season: str) -> pd.DataFrame:
    """League-wide advanced team table for rankings on League tab."""
    try:
        frames = _retry_api(
            leaguedashteamstats.LeagueDashTeamStats,
            dict(
...
def player_dashboard():
    ...
    with st.sidebar:
        ...
        filtered_players = (
            season_players if not q else season_players[season_players["PLAYER_NAME"].str.contains(q, case=False, na=False)]
        )
    ...
    _rank_tile(
        c1,
        "DEF Rating",
        opp_row.get("DEF_RATING", np.nan),
        opp_row.get("DEF_RANK", np.nan),
        total=30,
        pct=False,
        decimals=1,
    )
    _rank_tile(
        c2,
        "PACE",
        opp_row.get("PACE", np.nan),
        opp_row.get("PACE_RANK", np.nan),
        total=30,
        pct=False,
        decimals=1,
    )
    _rank_tile(
        c3,
        "NET Rating",
        opp_row.get("NET_RATING", np.nan),
        opp_row.get("NET_RANK", np.nan),
        total=30,
        pct=False,
        decimals=1,
    )
    ...
    for col in ["MIN", "PTS", "REB", "AST", "FG3M"]:
        if col not in logs.columns:
            logs[col] = 0
    window_df = logs if st.session_state.get("recent_sel", "Season") == "Season" else logs.head(
        int(st.session_state["recent_sel"])
    )
    ...
    trend_df = logs[["GAME_DATE"] + trend_cols].head(
        int(n_recent_val) if n_recent_val != "Season" else len(logs)
    ).copy()
    ...
    # ----------------------- Per 36 — Career / Season / Recency / Opponent -----------------------
    ...
    if vs_opp_df.empty:
        ...
        if opp_abbrev:
            ...
            if not all_logs.empty and "MATCHUP" in all_logs.columns:
                ...
                vs_opp_df = all_logs[all_logs["OPP_ABBR"] == resolve_team_abbrev(opponent, opp_row)][
                    cols_base
                ].copy()
    ...
    if not vs_opp_df.empty:
        per36_rows.append(
            _series_from_dict(
                compute_per36_from_logs(vs_opp_df.head(5)),
    ...
    num_fmt = {
        c: "{:.1f}"
        for c in last5.select_dtypes(include=[np.number]).columns
        if c != "GAME_DATE"
    }
    st.dataframe(
    ...
    if vs_opp_df.empty:
        ...
        if "SEASON" in career_df.columns and not career_df.empty:
            ...
            season_labels = sorted(season_labels, key=_yr2, reverse=True)
        else:
            season_labels = SEASONS
        ...
        if opp_abbrev:
            ...
            if not all_logs.empty and "MATCHUP" in all_logs.columns:
                ...
                vs_opp_df = all_logs[all_logs["OPP_ABBR"] == opp_abbrev][cols_base].copy() if opp_abbrev else pd.DataFrame(
                    columns=cols_base
                )
    ...
    if vs_opp_df.empty:
        ...
        num_fmt2 = {
            c: "{:.1f}"
            for c in vs_opp5.select_dtypes(include=[np.number]).columns
            if c != "GAME_DATE"
        }
        st.dataframe(
    ...
    st.caption(
        "Notes: Opponent metrics are NBA-only ‘Regular Season’ through today’s ET date (5-min cache). "
        "MIN totals from Base (Totals); PACE/ratings from Advanced (PerGame). Per-36 are computed by summing totals first, then scaling to 36."
    )
...
def league_dashboard():
    ...
    with st.spinner("Loading standings..."):
        east, west = get_standings(season)

    cols = st.columns(2)
    with cols[0]:
        st.markdown("#### Eastern Conference")
        if east.empty:
            st.info("Unable to load Eastern Conference standings.")
        else:
            st.dataframe(
                east[["RANK", "ABBR", "W", "L", "WIN_PCT"]],
                use_container_width=True,
                height=_auto_height(east),
            )
    with cols[1]:
        st.markdown("#### Western Conference")
        if west.empty:
            st.info("Unable to load Western Conference standings.")
        else:
            st.dataframe(
                west[["RANK", "ABBR", "W", "L", "WIN_PCT"]],
                use_container_width=True,
                height=_auto_height(west),
            )

    # League rankings (team advanced)
    st.subheader("League Rankings — Team Advanced Metrics")
    with st.spinner("Loading team advanced metrics..."):
        adv = get_league_team_advanced(season)

    if adv.empty:
        st.info("Unable to load league advanced metrics for this season.")
        return

    adv = adv.copy()
    adv["OFF_R_RANK"] = adv["OFF_RATING"].rank(ascending=False, method="min").astype(int)
    adv["DEF_R_RANK"] = adv["DEF_RATING"].rank(ascending=True, method="min").astype(int)
    adv["NET_R_RANK"] = adv["NET_RATING"].rank(ascending=False, method="min").astype(int)
    adv["PACE_RANK"] = adv["PACE"].rank(ascending=False, method="min").astype(int)

    # Top 10 tables
    st.markdown("##### Top 10 Offensive Rating")
    top_off = adv.sort_values("OFF_RATING", ascending=False).head(10)[
        ["OFF_R_RANK", "TEAM_ABBREVIATION", "OFF_RATING", "PACE"]
    ]
    st.dataframe(
        top_off.rename(columns={"OFF_R_RANK": "RANK", "TEAM_ABBREVIATION": "TEAM"}).round(2),
        use_container_width=True,
        height=_auto_height(top_off),
    )

    st.markdown("##### Top 10 Defensive Rating (lower is better)")
    top_def = adv.sort_values("DEF_RATING", ascending=True).head(10)[
        ["DEF_R_RANK", "TEAM_ABBREVIATION", "DEF_RATING", "PACE"]
    ]
    st.dataframe(
        top_def.rename(columns={"DEF_R_RANK": "RANK", "TEAM_ABBREVIATION": "TEAM"}).round(2),
        use_container_width=True,
        height=_auto_height(top_def),
    )

    st.markdown("##### Top 10 Net Rating")
    top_net = adv.sort_values("NET_RATING", ascending=False).head(10)[
        ["NET_R_RANK", "TEAM_ABBREVIATION", "NET_RATING", "OFF_RATING", "DEF_RATING"]
    ]
    st.dataframe(
        top_net.rename(columns={"NET_R_RANK": "RANK", "TEAM_ABBREVIATION": "TEAM"}).round(2),
        use_container_width=True,
        height=_auto_height(top_net),
    )

    st.caption(
        "All league data is pulled live from NBA.com endpoints via nba_api and cached to reduce load and rate-limit issues."
    )
